...
<behavior name="llm_first_inference" priority="critical">
For ambiguous or semantic classification tasks, prefer LLM inference with typed structured output over hand-written heuristics.

Rules:
- Use deterministic code for parsing/validation/transforms.
- Use LLMs for meaning, role, intent, and contextual inference.
- Default to typed outputs (e.g., Pydantic schemas) so behavior is explicit and testable.
</behavior>

<behavior name="no_fallbacks_fail_loudly" priority="critical">
Do not implement silent fallbacks that mask failures in core logic.

Rules:
- If a critical inference step fails, raise an error with actionable context.
- Do not substitute guessed/default behavior to "keep things running."
...
13. Replacing LLM inference with brittle heuristics for semantic tasks
14. Adding silent fallbacks that hide bugs instead of failing loudly
</failure_modes_to_avoid>
